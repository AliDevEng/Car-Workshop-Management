...
/**
 * The one search box in the top bar (PROJECT_SPEC.md §6.3, B3.4).
 *
 * Customers and vehicles now; articles are added in B4.6 once `Article`
 * exists. Each category is capped so one crowded kind cannot fill the list,
 * and the result is a discriminated union so the frontend's per-kind switch is
...
export async function search(db: Database, q: string): Promise<SearchResponse> {
  ...
  return { query: q, results: [...customers, ...vehicles] };
}
