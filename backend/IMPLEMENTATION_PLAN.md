...
> Package architecture, local setup, commands and conventions are summarized in
...
5. Mark the iteration **Done** after its checklist and Definition of Done pass.
   Record evidence/date and update the root README's progress snapshot and phase
   status when the totals change.
...
backup, and the elapsed time is written into this iteration's completion record.
...
- [x] **B12.5.2** Verify a work order, a document file and its hash all
      ...
      (`f16e3af274f063406b7f3cbd7e04f752daa7c15b703795d120e657510dd7a54e`).
- [x] **B12.5.3** Record the date and elapsed time in this iteration's
      completion record.
...
[F8.8](../frontend/IMPLEMENTATION_PLAN.md#f8-8) — the dialog opening in an
error state, and
...
in [`CLAUDE.md`](../CLAUDE.md) — always article rows before work order rows —
so that deadlocks are designed out rather than debugged later.
